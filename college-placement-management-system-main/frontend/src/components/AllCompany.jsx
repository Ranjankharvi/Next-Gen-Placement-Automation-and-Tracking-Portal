...
function AllCompany() {
  document.title = "DSCE | All Company";
  ...
  const confirmDelete = async (companyId) => {
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/company/delete-company`,
        { companyId },
        {
          headers: {
            Authorization: `Bearer ${token}`,
                ...
                {companys?.length > 0 ? (
                  companys?.map((company, index) => (
                    <tr key={company?._id}>
                      ...
                      <td>
                        {jobs.length
                          ? jobs?.filter((job) => job?.company == company?._id)
                              ?.length
                          : ""}
                      </td>
                      <td>
                          ...
                          <div className="px-0.5">
                            ...
                            >
                              <i
                                ...
                                onClick={() => {
                                  if (currentUser === "tpo_admin")
                                    navigate(
                                      `../tpo/add-company/${company._id}`
                                    );
                                  else if (currentUser === "management_admin")
                                    navigate(
                                      `../management/add-company/${company._id}`
                                    );
                                  else if (currentUser === "superuser")
                                    navigate(
                                      `../admin/add-company/${company._id}`
                                    );
                                }}
                                onMouseEnter={(e) => {
