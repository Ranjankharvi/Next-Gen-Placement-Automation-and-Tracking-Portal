...
function UpdateJobStatus() {
  document.title = "DSCE | Update Job Application Status";
  ...
  const fetchJobDetailsOfApplicant = async () => {
    if (data?.applicants?.length !== 0) {
      ...
      const appliedApplicant = await data.applicants.find(
      ...
      if (appliedApplicant)
        setApplicant(
          appliedApplicant
        ); // If no applicant found, navigate to 404
      else navigate("../404");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/student/update-status/${jobId}/${currentUser.id}`,
        { applicant }
      );
  ...
  const handleApplicantChange = (e) => {
    ...
    if (e.target.name === "status" && e.target.value === "hired")
      setHired(true);
    if (e.target.name === "status" && e.target.value !== "hired")
      setHired(false);
  };
  ...
  const confirmDelete = async () => {
    try {
      const response = await axios.post(
        `${BASE_URL}/student/delete-offer-letter/${jobId}/${currentUser.id}`,
        { applicant }
      );
      // console.log(response.data);
      if (response?.data?.msg) {
            ...
            <div className="flex flex-col gap-2">
              <div className="">
                ...
                <Accordion
                  defaultActiveKey={["0"]}
                  alwaysOpen
                  className="shadow rounded w-full max-sm:w-fit"
                >
                  ...
                  <Accordion.Body>
                    <div className="">
                      <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
                        {/* current round  */}
                        <FloatingLabel
                          controlId="floatingSelectCurrentRound"
                          label="Current Round"
                        >
                          <Form.Select
                        ...
                        {/* round status  */}
                        <FloatingLabel
                          controlId="floatingSelectRoundStatus"
                          label="Round Status"
                        >
                          <Form.Select
                        ...
                        {/* selection date */}
                        <FloatingLabel
                          controlId="floatingSelectionDate"
                        ...
                        <div className="flex flex-col gap-2 justify-center items-center">
                          ...
                          {applicant?.offerLetter && (
                            <div className="cursor-pointer w-fit flex gap-1 justify-between items-center">
                              ...
                              >
                                <i className="fa-regular fa-trash-can pr-2 py-1 text-lg" />
                                Delete
                              </span>
                            </div>
                          )}
                        ...
                        {/* job status  */}
                        <FloatingLabel
                          controlId="floatingSelectJobStatus"
                          label="Job Status"
                        >
                          <Form.Select
