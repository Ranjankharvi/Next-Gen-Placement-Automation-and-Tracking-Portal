...
function Navbar({ isSidebarVisible, toggleSidebar }) {
  ...
  return (
    <div
      className={`h-20 sticky top-0 z-10 bg-white flex justify-start items-center border-b-2 border-gray-100 shadow-sm text-gray-500 transition-all duration-300 ${
        isSidebarVisible ? "ml-60 px-4" : "ml-0"
      }`}
    >
      <button className="ml-4" onClick={toggleSidebar}>
        <FaBars size={24} />
      </button>
      <span className="ml-8 text-xl">{pageName}</span>
    </div>
  );
}
